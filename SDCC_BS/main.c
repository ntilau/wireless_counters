  ...
  while(1) {
    ...
    if(fifo[0] == 0x5A)
    {
      ...
      if(checksum == fifo[5])
      {
        ...
        {
          case 1: 
            for(idx=0; idx<4; idx++) Port1[idx]+=fifo[idx+1];
          case 2:
            for(idx=0; idx<4; idx++) Port2[idx]+=fifo[idx+1];
          case 3: 
            for(idx=0; idx<4; idx++) Port3[idx]+=fifo[idx+1];
          case 4:
            for(idx=0; idx<4; idx++) Port4[idx]+=fifo[idx+1];
        }
        LED = 0;
      }
    }
