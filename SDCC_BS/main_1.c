...
uchar fifo[100];
uchar fifoIdx = 0;
uchar dataTx[] = {0x2D, 0xD4, 0x33, 0x33};
...
void main() {
  ...
  // wr_command(0xA6, 0x80); // RF frequency @ 868.32MHz
  // wr_command(0xC6, 0x23); // BaudRate over the air: 9578.544 bps
  //wr_command(0xC6, 0x05); // BaudRate over the air: 57.6kbps
  //wr_command(0x96, 0x61); // Receiver control command. VDI slow + 270 kHz + RSSI @ -85dBm
  // wr_command(0xC2, 0xEE); // Data Filter. Automode + digitalRC + last nibble : C->F
  wr_command(0xCA, 0x82); // FIFO fill when Synchon Pattern received. 2 bytes Synchon
  wr_command(0xCE, 0xA5); // Set 2nd byte to 0xA5
  // wr_command(0xC4, 0xF7); // AFC (+ strobe edge for calculation) -> Pairing with BS
  //wr_command(0x98, 0xA0); // 165kHz, MAX Pout
  // wr_command(0xCC, 0x02); // PLL Setting command - as on POR
  // wr_command(0xE1, 0x96); // WakeUp timer left POR value 309.5 ms
  // wr_command(0xC8, 0x0E); // DutyCycle left POR value 10%
  // wr_command(0xC0, 0x00); // POR values of LowBat 2.2V and MCUclk 1MHz
  // wr_command(0xFE, 0x00); // Reset
  ...
  while(1) {
    ...
    while(FFIT){
      nFFS = 0;
      LED = 1;
      fifo[fifoIdx++] = spi_trx(0xFF); //= fifo_read();//
    ...
    //wr_command(0xCA, 0x80);
    //wr_command(0xCA, 0x82);
